...
/** Mean of `vals` over the entries whose distance lies in [d0, d1]. Nulls are
 *  skipped; an empty band returns null rather than 0, because 0 luma is a
 *  legitimate reading and "no samples" is not. */
export function bandMean(vals, ds, d0, d1) {
  let s = 0, n = 0;
  for (let i = 0; i < vals.length; i++) {
    if (ds[i] < d0 || ds[i] > d1 || vals[i] === null || vals[i] === undefined) continue;
    s += vals[i]; n++;
  }
  return n ? s / n : null;
}

/** The steepest single step in `vals` within `dmax` of the base, and where it
 *  is. This is the "umbra edge" quantity: a hard shadow boundary is a large
 *  step over one 0.05 m sample, a wash is a small one everywhere. */
export function maxStep(vals, ds, dmax) {
  let best = 0, at = null;
  for (let i = 1; i < vals.length; i++) {
    if (ds[i] > dmax) break;
    if (vals[i] === null || vals[i - 1] === null) continue;
    const g = Math.abs(vals[i] - vals[i - 1]);
    if (g > best) { best = g; at = ds[i]; }
  }
  return { step: best, at };
}

/**
 * Reduce one prop's two rays to the numbers the ring average could not make.
 *
 *   umbra  mean luma on the shadow side of the base over mean luma 1.8-2.5 m
 *          out along the SAME ray. Below 1 the prop is darkening the ground it
 *          stands on. Same ray, so the exposure stop and the surface albedo
 *          both cancel.
 *   asym   sun-side minus shadow-side over the same 0.30 m of base. A cast
 *          shadow is positive; a radially symmetric AO wash is 0 whatever its
 *          depth. This is the term a 32-point ring averages away.
 *   edge   the steepest 0.05 m step on the shadow side inside 1.2 m.
 */
export function reduceRays(r) {
  const { ds, away, toward } = r;
  const nearA = bandMean(away, ds, 0, 0.30);
  const nearT = bandMean(toward, ds, 0, 0.30);
  const openA = bandMean(away, ds, 1.8, 2.5);
  const e = maxStep(away, ds, 1.2);
  const f2 = (v) => (v === null ? null : +v.toFixed(2));
  return {
    kind: r.kind, dist: r.dist, resolved: r.resolved, pxPerM: r.pxPerM, shadowLenM: r.shadowLenM,
    nearAway: f2(nearA), nearToward: f2(nearT), openAway: f2(openA),
    umbra: nearA === null || !(openA > 0) ? null : +(nearA / openA).toFixed(4),
    asym: nearA === null || nearT === null ? null : +(nearT - nearA).toFixed(2),
    edge: +e.step.toFixed(2), edgeAt: e.at,
  };
}
...
function selftest() {
  ...
  // ---- the directional reduction, on inputs whose right answer is known.
  const ds = [];
  for (let d = 0; d <= 2.5001; d += 0.05) ds.push(+d.toFixed(2));
  const flatA = ds.map(() => 100), flatT = ds.map(() => 100);

  say('bandMean averages only its band',
    bandMean(ds.map((d) => (d <= 0.3 ? 50 : 150)), ds, 0, 0.30) === 50);
  say('bandMean skips nulls', bandMean([null, 10, null, 20], [0, 0.05, 0.1, 0.15], 0, 1) === 15);
  say('an empty band is null, not zero', bandMean([1, 2], [9, 9], 0, 1) === null);

  // KNOWN-BAD INPUT 5: FLAT GROUND. A prop that darkens nothing must read
  // umbra 1, asym 0, edge 0. If any of those comes back looking like contact,
  // every number this tool prints is worthless.
  const flatR = reduceRays({ ds, away: flatA, toward: flatT });
  say('flat ground: umbra 1.0', flatR.umbra === 1, `got ${flatR.umbra}`);
  say('flat ground: asym 0', flatR.asym === 0, `got ${flatR.asym}`);
  say('flat ground: edge 0', flatR.edge === 0, `got ${flatR.edge}`);

  // KNOWN-BAD INPUT 6: A RADIALLY SYMMETRIC WASH -- exactly the "soft blob"
  // the reviewers complained about. It is darker at the base on BOTH sides and
  // decays smoothly. umbra must show the darkening, asym must stay 0 (it has no
  // direction) and edge must stay small (it has no boundary). A metric that
  // cannot separate this from a cast shadow is the metric this tool replaced.
  const wash = ds.map((d) => 100 - 40 * Math.exp(-d / 0.8));
  const w = reduceRays({ ds, away: wash, toward: wash.slice() });
  say('wash: umbra registers the darkening', w.umbra < 0.75, `got ${w.umbra}`);
  say('wash: asym is 0 (no direction)', w.asym === 0, `got ${w.asym}`);
  // 40 * (1 - exp(-0.05/0.8)) = 2.42 is this wash's analytic steepest step, so
  // the bound is 3 and not something rounder: a threshold set below the right
  // answer fails the tool instead of the input.
  say('wash: edge stays small (no boundary)', w.edge < 3.0, `got ${w.edge}`);

  // KNOWN-GOOD INPUT: a hard-edged cast shadow 0.6 m long on the away side
  // only. asym and edge must both fire, which is what tells it from the wash.
  const cast = ds.map((d) => (d < 0.6 ? 45 : 100));
  const c2 = reduceRays({ ds, away: cast, toward: flatT });
  say('cast shadow: umbra registers', c2.umbra < 0.5, `got ${c2.umbra}`);
  say('cast shadow: asym fires where the wash did not', c2.asym > 50, `got ${c2.asym}`);
  say('cast shadow: edge finds the boundary', c2.edge > 50 && Math.abs(c2.edgeAt - 0.6) < 1e-9,
    `got ${c2.edge} at ${c2.edgeAt}`);
  // and the discriminator itself, stated as the assertion it is
  say('wash and cast are DISTINGUISHED (edge)', c2.edge > 10 * Math.max(w.edge, 0.1),
    `cast ${c2.edge} vs wash ${w.edge}`);

  // KNOWN-BAD INPUT 7: an all-null ray (every sample rejected as occluded) must
  // be null, not a confident 1.0.
  const nulls = reduceRays({ ds, away: ds.map(() => null), toward: flatT });
  say('an all-null ray gives null, not 1.0', nulls.umbra === null && nulls.asym === null,
    `got umbra ${nulls.umbra} asym ${nulls.asym}`);
...
const out = await page.evaluate(async ([armNames, boxes, minpx, wpx, hpx, wantShots]) => {
  ...
  for (const c of cand) {
    ...
    // ---- THE RING AVERAGE IS BLIND TO THE THING UNDER TEST, and that is not a
    // refinement, it is the reason the ratio table below reads 1.06 and 1.45 for
    // props that are plainly standing on something.
    //
    // A cast shadow is DIRECTIONAL. At noon it lies on one side of the prop and
    // the other side is open sunlit paving, so a mean over 32 points of a circle
    // averages the umbra against its own complement and returns the background.
    // Worse, it is not even a clean average: the sunlit side is brighter than the
    // 2.2 m reference ring (which is partly in building shade), so the ratio can
    // and does come out ABOVE 1 for a prop with a perfectly good shadow.
    //
    // So the ground is sampled along two RAYS instead: one running away from the
    // sun in plan, which is where the shadow is, and one running toward it, which
    // is the control. Each step averages a short transverse span so a single
    // stray pixel cannot carry it, but the span is perpendicular to the ray and
    // therefore cannot smear the thing being resolved. The difference between the
    // two rays is a quantity a ring cannot produce and a wash cannot fake.
    const rayAt = (ux, uz) => {
      const out = [];
      const px_ = -uz, pz_ = ux;                       // unit perpendicular in plan
      for (let d = 0.0; d <= 2.501; d += 0.05) {
        const pts = [];
        for (const t of [-0.10, -0.05, 0, 0.05, 0.10]) {
          const x = c.x + ux * (c.rM + d) + px_ * t, z = c.z + uz * (c.rM + d) + pz_ * t;
          const g = probe(x, z);
          if (!g || onProps(g) || Math.abs(g.y - c.groundY) > 0.30) continue;
          const p = project(x, g.y, z);
          if (p.z > 1 || p.x < 6 || p.x > wpx - 6 || p.y < 6 || p.y > hpx - 6) continue;
        ...
        out.push({ d: +d.toFixed(2), pts });
      }
      return out;
    };
    const sd = new THREE.Vector3().copy(D.tod.sun.position).sub(D.tod.sun.target.position);
    const sl = Math.hypot(sd.x, sd.z) || 1;
    const sx = sd.x / sl, sz = sd.z / sl;              // horizontal unit vector TOWARD the sun
    const away = rayAt(-sx, -sz), toward = rayAt(sx, sz);

    subjects.push({ kind: c.kind, dist: +c.dist.toFixed(1), rM: c.rM, hM: c.hM, prof, away, toward,
      ...
      // How long the prop's own shadow is on the ground, so a null result at a
      // 76-degree sun can be told apart from a null result at all.
      shadowLenM: +(c.hM / Math.max(1e-3, Math.tan(Math.asin(Math.max(-1, Math.min(1, sd.y / (sd.length() || 1))))))).toFixed(2),
  ...
  const saved = {
    ...
    aoScale: D.post.params.aoScale, aoBlurRadius: D.post.params.aoBlurRadius,
  ...
  const reset = () => {
    ...
    if (D.furniture && saved.shadowReach !== null && D.furniture._shadowReach !== saved.shadowReach) {
      D.furniture._shadowReach = saved.shadowReach;
      D.furniture.cullProps(cam.position);
    }
  ...
  const APPLY = {
    ...
    // ---- THE BUFFER THE AO IS DRAWN INTO, which none of the three candidates
    // in the brief names. aoScale is the AO target's fraction of the frame and
    ...
    aoFullBlur1: () => { D.post.params.aoScale = 1.0; D.post.params.aoBlurRadius = 1; resize(); },
    aoFullBlur0: () => { D.post.params.aoScale = 1.0; D.post.params.aoBlurRadius = 0; resize(); },
    // ---- THE SHADOW-REACH CULL, candidate 3. Not "turn the cull off" in the
    // abstract: set the reach past every bucket in the world, so if the cull is
    // withholding a caster that would have reached the frame, this arm restores
    // it and the frame moves. cullProps() is what actually sets castShadow, so
    // the reach is changed and cullProps re-run rather than castShadow poked.
    reachInf: () => {
      if (!D.furniture) return;
      D.furniture._shadowReach = 1e9;
      D.furniture.cullProps(cam.position);
    },
  };
  // Changing the AO buffer's size means rebuilding the render targets, which
  // post.setSize() does off the size it was last given.
  const resize = () => { const s = D.post._size; if (s) D.post.setSize(s[0], s[1]); };
  ...
  for (const name of armNames) {
    ...
    results[name] = {
      ...
      // The two rays, RAW. Everything derived from them -- the umbra ratio, the
      // sun-side asymmetry, the edge gradient -- is computed in the Node half by
      // exported functions, so --selftest can drive the whole reduction on
      // known-bad input without a browser. See the note beside rayAt().
      rays: subjects.map((s) => ({
        kind: s.kind, dist: s.dist, resolved: s.resolved, pxPerM: s.pxPerM,
        shadowLenM: s.shadowLenM,
        ds: s.away.map((r) => r.d),
        away: s.away.map((r) => { const v = r.pts.length ? ringLuma(px, r.pts) : null; return v === null ? null : +v.toFixed(1); }),
        toward: s.toward.map((r) => { const v = r.pts.length ? ringLuma(px, r.pts) : null; return v === null ? null : +v.toFixed(1); }),
      })),
  ...
  sun.shadow.needsUpdate = true;
  {
    ...
    results.__repeat = {
      ...
      // The repeat has to carry the DIRECTIONAL rays too, or the guard is
      // checking a number nobody quotes any more.
      rays: subjects.map((s) => ({
        kind: s.kind, dist: s.dist, resolved: s.resolved, pxPerM: s.pxPerM, shadowLenM: s.shadowLenM,
        ds: s.away.map((r) => r.d),
        away: s.away.map((r) => { const v = r.pts.length ? ringLuma(px, r.pts) : null; return v === null ? null : +v.toFixed(1); }),
        toward: s.toward.map((r) => { const v = r.pts.length ? ringLuma(px, r.pts) : null; return v === null ? null : +v.toFixed(1); }),
      })),
...
out.subjects.forEach((s, i) => {
  ...
  // The umbra ratio is the number this tool now reports, so it is the number
  // the guard has to protect. A run whose ring ratios repeat but whose umbra
  // does not is a run where something moved on one side of the prop only.
  const ra = out.results.base.rays && out.results.base.rays[i];
  const rb = rep.rays && rep.rays[i];
  if (ra && rb) {
    const ua = reduceRays(ra).umbra, ub = reduceRays(rb).umbra;
    if (ua !== null && ub !== null) pairs.push([ua, ub, `umbra ${s.kind}@${s.dist}m`]);
  }
});
...
// ---- THE DIRECTIONAL TABLE. The ring ratio above cannot tell a cast shadow
// from an AO wash and cannot tell either from nothing, because it averages the
// shadow side against the sunlit side. These three columns can:
//   umbra  shadow-side base luma / same-ray luma 1.8-2.5 m out.  <1 = grounded
//   asym   sunlit-side base minus shadow-side base.              >0 = directional
//   edge   steepest 0.05 m step on the shadow side within 1.2 m. >0 = a boundary
const DIR = {};
for (const n of [...ARMS, '__repeat']) {
  const rr = out.results[n];
  if (!rr || rr.error || !rr.rays) continue;
  DIR[n] = rr.rays.map(reduceRays);
}
for (let si = 0; si < out.subjects.length; si++) {
  const s = out.subjects[si];
  const any = DIR[ARMS[0]] && DIR[ARMS[0]][si];
  if (!any) continue;
  console.log(`\ndirectional contact — ${s.kind}@${s.dist} m  (${s.pxPerM} px/m, ` +
    `0.05 m = ${(0.05 * s.pxPerM).toFixed(1)} px, own shadow ${any.shadowLenM} m long, ` +
    `${s.resolved ? 'RESOLVED' : 'UNRESOLVED'})`);
  console.log('  arm'.padEnd(26) + 'umbra'.padStart(9) + 'asym'.padStart(9) + 'edge'.padStart(9)
    + 'edgeAt'.padStart(9) + 'nearAway'.padStart(10) + 'openAway'.padStart(10));
  for (const n of [...ARMS, '__repeat']) {
    const d = DIR[n] && DIR[n][si];
    if (!d) continue;
    console.log('  ' + n.padEnd(24) + String(d.umbra ?? '-').padStart(9) + String(d.asym ?? '-').padStart(9)
      + String(d.edge ?? '-').padStart(9) + String(d.edgeAt ?? '-').padStart(9)
      + String(d.nearAway ?? '-').padStart(10) + String(d.openAway ?? '-').padStart(10));
  }
}
